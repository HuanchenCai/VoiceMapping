...
STRINGS: dict[str, dict[str, str]] = {
    "zh": {
        ...
        # IAIF voice-derived analogs（无 EGG 通道时通过反演 glottal flow 算出的等价量）
        "metric.desc.Qcontact_voice":   "声门接触商（嗓音反演）。无 EGG 时由 glottal flow 估计，含义同 Qcontact。",
        "metric.desc.dEGGmax_voice":    "声门关闭瞬间的最大速度（嗓音反演）。无 EGG 时由 glottal flow 导数估计。",
        "metric.desc.Icontact_voice":   "接触强度指数（嗓音反演）。无 EGG 时由 glottal flow 估计。",
        "metric.desc.Entropy_voice":    "样本熵（嗓音反演）。基于 glottal flow 的逐周期 DFT，反映周期间稳定性。",
        "metric.desc.HRFegg_voice":     "谐波丰度（嗓音反演）。基于 glottal flow 的频谱估计。",
        "metric.desc.OQ_voice":         "开商（嗓音反演）。无 EGG 时由 glottal flow 时序估计。",
        "metric.desc.SPQ_voice":        "开闭速度商（嗓音反演）。无 EGG 时由 glottal flow 时序估计。",
        "metric.desc.CIQ_voice":        "接触不对称指数（嗓音反演）。无 EGG 时由 glottal flow 时序估计。",
        ...
        "metric.tooltip.ZCR":               "过零率：ZCR = (1/N)·Σ 1{x[n]·x[n+1] < 0}，每周期内信号过零次数除以周期长度。反映高频成分含量：浊音段低（< 0.05），清音/辅音段高（> 0.2）。",
    ...
    "en": {
        ...
        # IAIF voice-derived analogs of the EGG-shape metrics (mono-mode only)
        "metric.desc.Qcontact_voice":   "Glottal contact quotient (voice-derived). Estimated from the reconstructed glottal flow when no EGG channel is available.",
        "metric.desc.dEGGmax_voice":    "Peak closure speed (voice-derived). Estimated from the glottal flow derivative when no EGG channel is available.",
        "metric.desc.Icontact_voice":   "Contact intensity (voice-derived). Estimated from the glottal flow when no EGG channel is available.",
        "metric.desc.Entropy_voice":    "Sample entropy (voice-derived). Computed on per-cycle DFT of the glottal flow.",
        "metric.desc.HRFegg_voice":     "Harmonic richness (voice-derived). Estimated from the spectrum of the reconstructed glottal flow.",
        "metric.desc.OQ_voice":         "Open quotient (voice-derived). Estimated from glottal-flow timing when no EGG is available.",
        "metric.desc.SPQ_voice":        "Speed quotient (voice-derived). Estimated from glottal-flow timing when no EGG is available.",
        "metric.desc.CIQ_voice":        "Contact-asymmetry index (voice-derived). Estimated from glottal-flow timing when no EGG is available.",
        ...
        "metric.tooltip.ZCR":                "Zero-Crossing Rate: ZCR = (1/N)·Σ 1{x[n]·x[n+1] < 0}, sign-changes per cycle divided by cycle length. Reflects high-frequency content: voiced low (< 0.05); unvoiced / consonants high (> 0.2).",
