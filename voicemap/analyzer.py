...
class VoiceMapAnalyzer:
    ...
    def calculate_all_metrics(self, voice_signal, egg_signal,
        ...
        # _STAGE_LABELS (so re-ordering / adding a stage is a 1-line change).
        def _step(abs_step: int):
            if progress_cb is not None:
                progress_cb(abs_step, self.TOTAL_STAGES,
                            self._stage_label(abs_step))
        ...
        # Note: an earlier version computed `*_voice` analogs (Qcontact_voice
        # etc.) by feeding -glottal_flow into the EGG calculators. Removed
        # 2026 because (a) only 3 / 8 had useful agreement with EGG
        # ground truth on real recordings, and (b) the EGG-shape calculators
        # are not the right model for the voice waveform — see the
        # `diagnose_voice_iaif.py` script for the parity audit that
        # motivated the removal. Without EGG those metrics are skipped
        # entirely; acoustic-only mode greys them out in the GUI.
    ...
    def output_vrp_csv(self, metrics: Dict[str, np.ndarray],
        ...
        return write_vrp(self, metrics,
                          ...
                          export_plots=export_plots, write_disk=write_disk)

    # ------------------------------------------------------------------
    # Full pipeline
    # ------------------------------------------------------------------
    # ── Pipeline stages — single source of truth ─────────────────────────
    # If you add a new metric / stage, INSERT IT HERE and update the
    # corresponding _step index in calculate_all_metrics or
    # analyze_and_output_vrp. The count, label, and progress bar bounds
    # all derive from this list so nothing else needs touching.
    _STAGE_LABELS = (
        "加载音频",                          # 1
        "预处理 (HPF / 带通滤波)",           # 2
        "周期检测 (phase-portrait)",         # 3
        "EGG DFT (每周期)",                  # 4
        "SPL / Clarity / CPP",               # 5
        "SpecBal / Crest",                   # 6
    ...
    @classmethod
    def _stage_label(cls, step: int) -> str:
        """Look up a stage label by 1-based index."""
        if 1 <= step <= len(cls._STAGE_LABELS):
            return cls._STAGE_LABELS[step - 1]
        return f"step {step}"
    ...
    def analyze_and_output_vrp(self, file_path: Optional[str] = None,
        ...
        def _cb(step):
            if progress_cb is not None:
                progress_cb(step, total, self._stage_label(step))
