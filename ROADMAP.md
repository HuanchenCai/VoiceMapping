...
- [ ] **M5.5** — Audition 式文件管理面板（多文件 load / 切换 / 批操作）
...
### M5.5 — 文件管理面板（参考 Adobe Audition 布局）

让 GUI 从"一次拖一个 wav"升级到"批量管理 + 切换"工作流。

- 左侧（或左上）多一个 **Files** 面板：展示当前 session 加载的所有
  音频文件，每条显示文件名 + 采样率 + 时长 + 已分析 ✓ / 未分析 ○
- 拖一批 .wav 进来 → 全部入列，**第一个自动分析**，其余排队待选
- 单击列表项 → 切换右侧 voice map（**已分析的从内存恢复 DataFrame，
  没分析的就地分析**）
- 右键文件项 → 上下文菜单：分析 / 重新分析 / 导出 CSV / 导出 Excel /
  从面板移除 / 在资源管理器里显示 / 加入对比集
- 多选（Ctrl+click）：
  - 批量分析（队列化）
  - 批量导出
  - **跨文件叠加 voice map**（不同录音的同一 metric 透明叠在一张图上，
    每个文件一种颜色，类似 Audition 多轨道时间线）
- 底部状态栏：当前选中文件的元数据（采样率 / 通道数 / 时长 / cycles 数 /
  分析耗时）
- 持久化：关 GUI 时保存 session（最近文件 + 当前选中），下次启动恢复

依赖：M1 注册表已经支持多组 DataFrame 共存；M2 的多格式导出会被复用。
建议在 M5（场景预设）之后做，因为不同场景下 Files 面板的默认列宽 /
默认动作会不同（医疗用户更想看时长 / 文件名；歌手更想看是不是已聚类
过）。
