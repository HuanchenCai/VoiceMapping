...
## 2026-05-08 (晚上)  GUI 大量迭代抛光（用户连续回报 8+ 轮）

承上一条 M1 验证完结后，继续按用户反馈逐项修。本批 commit 5d2815c
→ 142580c，覆盖 i18n 漏洞、视觉 bug、UX 缺失、Compare dialog 横向
裁切等多个面：

### ✅ tested

**i18n 漏洞 + 切语言路径**
- 'header.metric' key 误用（zh+en 都写成 "Metric"），导致中文模式
  Metric Bar 标签变 "Metric"。修：用 'metric_bar.label'。(07f743d)
- '本次值' label 没加进 _safe_text，切英文不变。修：加键。(5c00507)
- '录音轨' / tracks.label 切英文不变。修：保存 widget 引用。(511634f)
- _THRESHOLDS 临床带 zh-only，新增 _THRESHOLDS_LABEL_EN 字典 +
  get_band_label 助手，render 时按当前语言查 EN。(511634f, 053b630)
- 23 个 metric 现在有参考范围（之前 15）—— 加 Crest / SpecBal /
  SpectralFlatness / AlphaRatio / HammarbergIndex / DUV / Entropy /
  SPR，每个 zh + EN 双语 band。(053b630)

**视觉 / 布局**
- 菜单栏切语言后跑到画布下方（_outer 错锚点）—— anchor 改 _header_frame。
  (511634f)
- 菜单栏下方 1 px BORDER 分隔线被判 "黑横条" —— 删掉。(511634f)
- ModernPopup add_separator 多轮迭代：BORDER #3a3a3a → PANEL_HI →
  no-op 不创建 widget。最终无任何视觉，分组靠间距 + cascade 箭头。
  (5c00507, 5d2815c, 7469609)
- Inspector "本次值" pill 大数字字号 22pt → 18pt → 22pt（恢复
  UI_DESIGN spec）。(9eb2620)
- range column 宽度 10 → 14（'0.13 – 0.19' 不再溢出到 label 列）。
  (9eb2620)
- status bar 高度 28 → 34（高 DPI 下 Microsoft YaHei UI 11pt 下行
  字符不再被裁）。(9eb2620)
- FONT_TITLE 18pt → 14pt 向 docs/UI_DESIGN.md spec 13pt 靠拢。
  (053b630)

**UX 改进**
- metric 名右上加 ⓘ glyph (U+24D8) 视觉提示可悬浮。(9eb2620)
- HoverTooltip 类（widgets.py），500 ms 延迟 + Toplevel + wraplength
  360 px。挂到 metric 名 + ⓘ。(63ed237 → 9eb2620)
- 25 条 metric.tooltip.<NAME> 详细 prose（zh + en），含具体公式
  （Σ / log10 / GCI / GOI 等），与 description（无公式只讲物理意义）
  形成两层：卡片正文给直觉，hover 给定义。(9eb2620)
- 80+ 条 metric.desc.<NAME> 短描述，全部物理直觉无公式。(63ed237)
- 导出 Excel / 报告完成后弹 3 选 1 对话框（_post_export_prompt）：
  打开文件 / 打开文件夹 / 不打开。Windows 用 explorer /select 高亮
  导出文件。(9eb2620)
- '上一个 metric ←' / '下一个 metric →' 改为可点击 Label 按钮。
  (5d2815c → 7469609 简化掉 'metric' 字)
- 文件菜单加 '添加文件…'（多选）+ '添加文件夹…'（rglob 递归）。
  CLI --batch 等价能力终于能从 GUI 走。(5d2815c)
- Tracks Panel 单行紧凑：双行 → 单行（filename + meta 同行），
  行高 ~52 px → 28 px，密度 ×2。(7469609)
- Tracks Panel 加滚动容器（tk.Canvas + Scrollbar + 145 px viewport）：
  超过 5 个 wav 自动出滚动条，鼠标滚轮在任何位置都触发 yview_scroll。
  (7469609)
- waveform 220×36 → 160×22 配合新行高。(7469609)
- CompareDialog 默认尺寸跟屏幕走（90% × 70%，封顶 1900×900），
  3 个子图 A | B | Δ 不会再被截。(142580c)

**重构 / 清理**
- Inspector 里 metric 描述查找改 tr(f"metric.desc.{name}") + 缺失
  fallback 到 spec.description（registry 英文）。(63ed237)
- Inspector 严重度 / coords pill 全部走 tr() 不再写死 'MIDI/SPL/dB' /
  'good/normal/watch/abnormal'。(4495871)
- metrics_registry 删 MFCC1-13（13 条菜单去除 + Icontact 去除）。
  CSV / 分析仍计算，仅 GUI 不展示。(e7d1a0f, 63ed237)
- Tracks panel 波形从 Unicode block 字符（' ▁▂▃▄▅▆▇█'）换成
  tk.Canvas 柱状图（_track_waveform_amps + _draw_waveform_canvas），
  安静段也有 1 px floor 不再断线。(4495871)

### 📝 doc + tests

- 单元测试 15 → 31（+107%）：(cf24a5f, 4bae756)
  - test_band_labels.py: 6 个 (zh/en 对称、未映射 fallback、batch-2
    metric 不丢)
  - test_metric_descriptions.py: 6 个 (REGISTRY ⊆ desc keys、zh⇔en
    对称、描述无公式、tooltip implies desc)
  - test_inspector_clinical.py: 4 个 (subTest 跑 23 个 metric 渲染、
    zh/en 显示验证、清空验证)
- validate_params.py 始终 48 PASS / 4 WARN / 0 FAIL

---

## 2026-05-08  Milestone 1 全部 🟡 untested 清零 + 8 个 metric 加参考范围

### ✅ tested

按 ROADMAP "Milestone 1 — 回填未测试功能验证"，4 个 🟡 项目全部跑过：

**M1.1 多 wav 联合 centroid 训练**（commit 待定）
3 wav（2 平铺 + 1 嵌套子目录）→ `--train-centroids out.csv a.wav b.wav c.wav`
→ 池化 38763 cycles fit K-means k=5 dim=27 → CSV 头 `# VoiceMap cluster
centroids` 格式正确 → 第 4 次分析用 `--load-centroids` 加载产物 CSV 跑通
12525 个有效点。

**M1.2 CompareDialog A | B | Δ 工作流**
两个 VRP CSV 走两条路：
- CLI `--compare A.csv B.csv --compare-metric CPP --compare-out out.png`
  生成 71 KB 三栏 PNG（A 热图 / B 热图 / A−B 差异）✓
- Headless GUI 实例化 CompareDialog，注入 DataFrames，自动求交集 80 个共同
  metric，render → 60 KB PNG ✓

**M1.3 Excel 导出**
80 metric × 1 sheet + Summary + Grouped = **82 个 sheet**。
openpyxl 读回检查：Summary 表头 metric/has_data/n_cells/mean/median/std/
min/max 都对，CPP 子表 SPL × MIDI 网格格式正确。无中文乱码（sheet 名都是
ASCII metric key），Excel/WPS 直接打开即可。

**M1.4 `--batch DIR` 端到端**
3 wav（含一层嵌套）→ `voicemap --batch /tmp/vm_batch --plot-mode none`
→ 3 个 CSV 落到输出目录。`Batch complete: 3 ok, 0 failed`。

### ✅ tested 增量

**8 个 metric 补参考范围**（_THRESHOLDS 从 15 → 23）：
- `Crest`：饱和(<1.4) / 典型(1.4-2) / 动态(2-3) / 瞬态(>3)
- `SpecBal`：明亮(<-10) / 平衡(±10) / 暗哑(>10)
- `SpectralFlatness`：纯音(<0.05) / 典型(0.05-0.30) / 噪声偏多(0.30-0.50) / 噪声主导(>0.50)
- `AlphaRatio`：紧张(<-10) / 正常(±10) / 松弛(>10)
- `HammarbergIndex`：压制(<10) / 正常(10-30) / 气声(>30)
- `DUV`：正常(<15%) / 中度断点(15-50%) / 高度断点(>50%)
- `Entropy`：高度规律(<0.3) / 正常(0.3-1.0) / 无序(>1.0)
- `SPR`：无歌者共振(<-25) / 中等(-25 至 -10) / 强烈(>-10)

每个 zh band label 都加 EN 翻译入 `_THRESHOLDS_LABEL_EN`，i18n 模式切换
后 23 个 metric 的 band 全部双语正常。`test_report_thresholds.py` 4 个
回归（severities valid / lo≤hi / 不重叠 / label nonempty）全过 ✓。

**字体层级对齐**：FONT_TITLE 18pt → 14pt bold，向 docs/UI_DESIGN.md
spec(13pt) 靠拢一点。Inspector metric name 仍 19pt，整体视觉权重
"Inspector 22(big number) > 19(metric) > Header 14(app title) > UI 11"
干净。

### 📝 doc

- `tests/validate_params.py`：48 PASS / 4 WARN / 0 FAIL（贯穿）
- 单元测试：15/15 全过
