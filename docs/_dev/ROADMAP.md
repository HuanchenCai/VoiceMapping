...
### M5.6 — 输入模式扩展（纯麦克风 / 多通道）

让软件不再强依赖「立体声 + EGG（通道 2）」的输入格式，覆盖
常见的纯麦克风录音场景。

**支持的三种输入模式**

- `stereo_egg`（默认 / 当前行为）：通道 1 = voice，通道 2 = EGG
- `stereo_voice_chN`：双声道两个都是 voice，用户在设置里选 ch1 或 ch2
- `mono_voice`：单声道纯 voice

**UX 期望**

- 设置面板里加输入模式 + voice 通道选择
- 没 EGG 的模式下，所有 EGG 类 metric / Inspector 临床卡 /
  报告章节 / Excel sheet / 菜单分类**完全隐藏**，不报错
- 用户切回 EGG 录音时自动恢复完整 metric 视图

**影响范围速查表**

| 模块 | 文件 | 关键行 | 改动类型 |
|---|---|---|---|
| 配置字段 | `voicemap/config.py` | `VoiceMapConfig` | 加 `input_mode` + `voice_channel_idx` 字段 |
| 音频载入 | `voicemap/analyzer.py` | `load_audio()` 行 310-319 | 按 `input_mode` 拆通道，没 EGG 时 `egg=None` |
| 周期切分 | `voicemap/analyzer.py` | `phase_portrait_cycle_detection()` 行 350-377 | 没 EGG 时走 voice-based 周期检测，或 frame 级 fallback |
| EGG calculator | `voicemap/metrics.py` | Qcontact / Icontact / dEGGmax / HRFegg / OQ / SPQ / CIQ / Entropy / Cluster / cPhon | `egg=None` 时跳过 |
| metric 注册 | `voicemap/metrics_registry.py` | `_populate_builtins` | 给 EGG 类 metric 加 `requires_egg=True` 标志 |
| metric 下拉 | `voicemap/gui/theme.py` `_METRIC_SECTIONS` + `voicemap/gui/app.py` `_metric_sections_avail` | 按 `input_mode` 动态过滤 EGG section |
| CSV / Excel / 报告 | `voicemap/csv_writer.py`、`voicemap/excel_export.py` `_METRIC_COLS`、`voicemap/report.py` `_emit_section("三、EGG …")` | 按 `df.columns` / `requires_egg` flag 动态裁剪 |
| 设置对话框 | `voicemap/gui/dialogs.py` `SettingsDialog` | 加输入模式 combobox + voice 通道选择 |

**算法选项**（v1.1 实现者决定）

- 选项 A：voice-based 周期检测（pYIN / SEDREAMS / DIO+StoneMask）
  —— 保留全套 per-cycle metric，准确度需 Praat 交叉验证
- 选项 B：frame-level fallback
  —— 只算 frame 级 metric（F0、SPL、Clarity、HNR、CPP、谱矩、
     MFCC、Formant、Vibrato），跳过 Jitter / Shimmer / 各种
     perturbation。改动小但 voice metric 覆盖率降低
...
- [x] 多 wav 联合 centroid 训练端到端：3 wav（含嵌套）池化 38763 cycles → 产物 CSV → 第 4 次分析正常 (DEV_LOG 2026-05-08)
- [x] `CompareDialog` 工作流：CLI + headless 双路验证，80 共同 metric 求交集 + render PNG (DEV_LOG 2026-05-08)
- [x] 真的导出一个 `.xlsx`：82 sheets（80 metric + Summary + Grouped），openpyxl 读回结构正确 (DEV_LOG 2026-05-08)
- [x] 批处理：3 wav 含嵌套 `--batch` → 3 CSV 落地 0 failed (DEV_LOG 2026-05-08)
