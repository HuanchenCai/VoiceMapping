...
- ✅ **Phase 5 — 论文 / 方法学包 (基本完成)**：5.1 ✅ `docs/methodology.md`（全 64 指标算法+公式+引用+验证状态，按类别组织，链到 metrics/*.md）；5.2 ✅ 合成语料打包 `scripts/package_corpus.py` → release zip（CC0）+ `corpora/synthetic.md`；5.3 ✅ `docs/reproducibility.md`（env + 一行命令复现表 + 确定性设计）；5.4 ✅ `docs/api_stability.md`（semver 1.0.0 冻结 + 公开 API 面）。
- 🟡 **Phase 6 — ML 集成 (核心完成，6.3 待语料)**：6.1 ✅ `voicemap/ml.py` VoiceFeatureExtractor（sklearn，audio/VRP→定长特征，130 维，名字从 registry 派生）；6.2 ✅ `docs/ml_schema.md`（脚本 `gen_ml_schema.py` 从 registry 自动生成）；6.4 ✅ read_vrp/parquet helper；6.5 ✅ `examples/ml_demo.py`（合成 3 类 → 特征 → 分类器，端到端）；6.3 ⏸ **预训练 centroid 库 deferred** —— EGG 训练/存/读机制已存在（train_cluster_centroids + --load-centroids），但「库」需多嗓 EGG 语料（现仅 1 个 stereo+EGG 文件，12 合成全 mono），且 cPhon 需加 preset-centroid 持久化；cluster 列已默认排除出 ML 特征，不阻塞。等语料到位再做。
...
| 10 | **Vibrato** (rate / extent / jitter) | Sundberg 1995 + commit d53b47b 修过 | ✅ PASS (合成 GT：rate ±0.3Hz、extent <5%)；零填充+W=80 已修 rate | ✅ docs/metrics/vibrato.md 8 节；§7 仅剩高 F0 时频残留 |
...
| VoicingRatio / DUV ✅ | PASS：(B) 合成 grid 机器精度恢复（VR=voiced/total、DUV=100·(1−VR)）+ (C) 真实录音 VR=1.0；metrics/integrative.md。**MPT 已删**（2026-06-01：广播标量、无每-cell 意义、非临床 MPT）|
