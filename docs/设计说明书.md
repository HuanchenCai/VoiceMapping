...
| SUCCESS / WARN / ERROR | 绿 / 橙 / 红 | 语义色（成功、警告、错误） |
...
| DROP | 13 | 拖入区主标题 |
| TITLE | 14 | 窗口标题 |
| INSPECTOR_NAME | 19 | 详情栏当前参数名 |
| DISPLAY | 22 | 详情栏大号数字 |
| MONO / MONO_B | 9 / 11 | 等宽（代码 / 数字） |
...
什么档位"。同时严重度色也参与多参数关联推断（如 Jitter 异常 +
Shimmer 异常 → 自动报告标"嗓音粗糙"）。
...
本软件的二维 Voice Mapping 方法学、聚类分析以及临床应用案例已在
下述同行评议期刊与预印本发表，软件实现与论文方法学一致：

- Lu Y. & **Cai H.** (2026). *A Voice Mapping Based Comparative
  Study of Vocal Characteristics in Bel Canto, Chinese National
  Singing and Popular Singing.* Journal of Voice. —— 美声 / 民族 /
  流行三种唱法的 Voice Map 对比研究，验证本软件在艺术嗓音研究中
  的适用性。
- **Cai H.** & Ternström S. (2026). *Voice Mapping of
  Text-to-Speech Systems: A Metric-Based Approach for Voice
  Quality Assessment.* arXiv preprint. —— 把 Voice Map 方法学
  推广到 TTS 嗓音质量评估，对应本软件的对比模式与多文件管理。
- **Cai H.** & Ternström S. (2025). *A WaveNet-Based Model for
  Predicting the Electroglottographic Signal from the Acoustic
  Voice Signal.* Journal of the Acoustical Society of America
  (JASA). —— EGG 与 voice 信号关系建模，是 v1.1 纯麦克风模式
  路线（§ M5.6）的理论基础。
- Iob N. A., He L., Ternström S., **Cai H.**, Brockmann-Bauser M.
  (2024). *Effects of Speech Characteristics on Electroglottographic
  and Instrumental Acoustic Voice Analysis Metrics in Women with
  Structural Dysphonia Before and After Treatment.* Journal of
  Speech, Language, and Hearing Research. —— EGG + 声学参数在
  结构性发音障碍治疗前后的临床指标研究，为本软件 EGG 类参数的
  临床阈值表提供了佐证。
- **Cai H.**, Ternström S., Chaffanjon P., Henrich Bernardoni N.
  (2024). *Effects on Voice Quality of Thyroidectomy: A Qualitative
  and Quantitative Study Using Voice Maps.* Journal of Voice. ——
  本软件 Voice Map 方法学在甲状腺手术前后嗓音质量评估中的临床
  应用案例。
- **Cai H.** & Ternström S. (2022). *Mapping Phonation Types by
  Clustering of Multiple Metrics.* Applied Sciences. —— 多参数
  K-means 聚类识别发声方式，对应本软件的 cPhon 聚类 / 跨录音
  centroid 训练机制。
